                                       ...
                                       @isset($main_categories)
                                        @foreach ($main_categories as $main_category)
                                                <tr>
                                                    ...
                                                    <td> {{ __('messages.'. $main_category->translation_lang) }}</td>
                                                    <td>{{ $main_category->getActive() }}</td>
                                                    <td> <img src="{{ asset('assets/'.$main_category->photo)  }} " class=" "  width="100px" height="60px"  alt=""></td>
                                                    <td>
                                                        <div class="btn-group" role="group"
                                                            aria-label="Basic example">
                                                            <a href="{{route('admin.maincategories.edit',$main_category -> id)}}"
                                                            class="btn btn-outline-primary btn-min-width box-shadow-3 mr-1 mb-1">تعديل</a>
                                                            <a href="{{route('admin.maincategories.delete',$main_category -> id)}}"
